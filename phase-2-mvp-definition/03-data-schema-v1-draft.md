...
状态：核心 Schema 取舍已确认并冻结 v1；不代表已开始建库
...
1. 第一版同时保存短证据文本和结构化 normalized_claim，不只保存 AI 总结；
2. 所有用于 P0 硬性雷点判断的证据必须经过人工审核；
3. 第一版暂不使用向量数据库，先使用作品 ID、结构化标签、雷点标签和关键词检索，后续根据漏召回数据决定是否增加 Embedding；
4. 保存匿名 AnalysisRun 日志，用于评测、成本和 Bad Case 复现，但不建立真实用户账号；
5. 用户反馈不能直接修改书籍事实，需要人工复核后再决定是否进入 Bad Case 或数据修正。
