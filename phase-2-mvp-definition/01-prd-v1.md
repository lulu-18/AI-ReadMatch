...
形成日期：2026-08-10
所有权：双方共创，用户最终决策
状态：开发基线已形成；标签配置、模型选择和评测阈值作为附属规格继续迭代，不阻塞基础工程启动

## 0. 文档使用说明

本文件是 ReadMatch MVP 的开发与评测基线。已冻结的用户、场景、范围和指标直接继承自项目负责人确认的 MVP 决策；后续修改必须记录证据、版本和影响范围。
...
面向纯爱读者的精选候选决策助手：先覆盖一批高讨论、高踩雷风险或观点分歧明显的作品，对已收录作品提供有证据的匹配、风险和未知分析。

#### 产品承诺

不保证覆盖所有书，只对已收录作品做有证据的判断；未收录作品不退回无证据的通用大模型自由回答。
...
第一版不预设大规模覆盖达标数值。用户测试和开放搜索记录用于建立基线，不能用已收录作品上的高准确率掩盖低搜索命中率。
## 5. MVP 范围
...
目的：帮助用户确认 ReadMatch 的支持范围并选择一本到已收录作品。
...
- 只能选择 `catalog_status=active` 的作品；
- 搜索结果同时显示书名和作者；
- 同名书必须要求用户确认作者或平台；
- 未收录作品不得进入 AI 分析；
- 未收录请求可以被聚合记录。
...
- 未选择正向偏好时仍可提交；
- 结果不虚构正向个性化匹配；
- 只显示基础事实、硬性雷点检查、风险和未知项。

### AC-04：证据检索

- 检索首先限定当前 `book_id`；
- 每个用户硬性雷点都必须执行检索或状态检查；
- 检索不到证据时输出 no_evidence/unknown，不输出 absent；
- 不允许引用其他作品的 evidence_id。

### AC-05：结构化生成

- 输出必须符合 AnalysisResult Schema；
- 每个匹配点和风险点必须关联 evidence_ids；
- 书名、作者、平台和完结状态由数据库提供；
- 模型不得补充输入中不存在的作品事实；
- 冲突观点必须保留，不允许平均成单一绝对结论。

### AC-06：硬性雷点检查

- 每个硬性雷点必须返回 present / possible / conflicting / unknown / no_evidence 中的一种状态；
- 发现 present 或高风险 possible 时，结果第一屏必须提示；
- 硬性雷点状态与综合判断矛盾时，规则阻止直接输出 likely_match。

### AC-07：规则校验与降级

- JSON 第一次失败时允许同输入重试一次；
- 第二次失败后降级显示基础事实和证据；
- 无证据结论必须删除或改为未知；
- 引用错书、硬性雷点遗漏或关键剧透泄露属于 P0 阻断；
- 降级和失败必须记录 AnalysisRun 与 BadCase。
...
- 未知项不得展示为“没有”；
- 关键剧透默认折叠；
- 用户可以查看结论对应的证据编号和来源类型。

### AC-09：用户反馈

- 用户可以选择试读、排除或继续核验；
- 用户可以反馈事实错误、雷点遗漏、匹配错误、证据不一致和剧透；
- 用户反馈不得自动修改 Book 或 Evidence；
- 高风险反馈进入人工复核和 BadCase。

### AC-10：日志、隐私与成本

- 保存匿名 session_id，不保存真实姓名、账号或联系方式；
- 保存 model、prompt_version、workflow_version、evidence_ids、延迟、Token 和成本；
- 原始问卷、完整书评和小说正文不进入产品数据库；
- `.env`、Token 和本地数据库不得上传 GitHub。
...
- 30 条核心评测 Case；
- 事实、证据、雷点、未知、延迟和成本比较；
- 至少 20 个项目负责人亲自审核的 Case/Bad Case。

### M3：精选书库和用户测试

- 扩充到约 30 本作品；
- 达到最低证据标准；
- 完成 1 名预测试和至少 4 名正式用户测试；
- 完成至少一轮产品和 Prompt 优化。
...
以下内容继续迭代，但不阻塞基础工程和 3 本样例端到端 Demo：

- 首屏最终偏好/雷点标签；
- 首批 30 本完整书单；
- 最终模型/API 选择；
- 是否增加 Embedding；
- 数据库最终使用 SQLite 还是 PostgreSQL；
- 视觉风格；
- 第一次 Baseline 后的指标阈值调整；
- 搜索命中率低时的长期产品转向。

这些待定项必须在进入相关里程碑前确认，不能永久悬而未决。
...
该 v1.0 是开发和评测使用的产品基线，不代表所有内容永久不变。后续修改必须记录原因、证据、版本和影响范围。
